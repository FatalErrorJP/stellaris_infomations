...
def _write_leader_trait_data(wf: TextIO) -> None:
    for key, data in LeaderTraits.get_data().items():
        wf.writelines(settings.TEMPLATE_DATA.format(
            ...
            rarity=_get_param('leader_trait_rarity', data),
            subclass_trait=_get_param('subclass_trait', data, 'no'),
            negative=_get_param('negative', data, 'no'),
            ...
            veteran_class_locked_trait=_get_param('veteran_class_locked_trait', data, 'no'),
